...
// quilt - Geometric quilt pattern with 8-way symmetry
// Ported from quilt.cc - draws pattern incrementally, one pixel group per frame
...
public struct QuiltDemo: Sendable {
    ...
    public static func run(options: Options, canvas: UDPFlaschenTaschen) async {
        let logger = Logger(subsystem: Logging.subsystem, category: "quilt")
