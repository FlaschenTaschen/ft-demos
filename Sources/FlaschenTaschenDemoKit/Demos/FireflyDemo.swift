...
// Firefly jar animation - ported from Dana's Arduino code
// Inspired by: https://www.instructables.com/Programmable-LED-Firefly-Jar/
...
public struct FireflyDemo: Sendable {
    ...
    public static func run(options: Options, canvas: UDPFlaschenTaschen) async {
        let logger = Logger(subsystem: Logging.subsystem, category: "firefly")

        logger.info("firefly: geometry=\(options.standardOptions.width, privacy: .public)x\(options.standardOptions.height, privacy: .public)+\(options.standardOptions.xoff, privacy: .public)+\(options.standardOptions.yoff, privacy: .public) layer=\(options.standardOptions.layer, privacy: .public) delay=\(options.standardOptions.delay, privacy: .public)ms lights=\(options.numLights, privacy: .public) pattern=\(options.patternName ?? "auto", privacy: .public) switch=\(options.patternSwitchSeconds, privacy: .public)s")
